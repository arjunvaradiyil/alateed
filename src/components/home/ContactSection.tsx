...
function Card({
  ...
  return (
    <div className="flex h-full min-h-[176px] flex-col border-t-2 border-brand-gold bg-[#f7f4ec] p-5 sm:min-h-[196px] sm:p-6">
      <div className="mb-4 flex h-12 w-12 items-center justify-center bg-brand-navy">{icon}</div>
      <h3 className="font-poppins mb-2 text-xs font-semibold uppercase tracking-wider text-brand-navy">
        {title}
      </h3>
...
export function ContactSection() {
  ...
  return (
    <div id="contact" className="scroll-mt-[var(--header-h)] overflow-x-hidden bg-white text-[#111]">
        ...
        <Container>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 xl:grid-cols-4">
            ...
            >
              <p>
                <a
                  href={`tel:${phone1.value.replace(/\s/g, '')}`}
                  className="font-bold transition-colors hover:text-brand-navy"
                  dir="ltr"
                >
                  {phone1.value}
                </a>
                <br />
                <a
                  href={`tel:${phone2.value.replace(/\s/g, '')}`}
                  className="font-bold transition-colors hover:text-brand-navy"
                  dir="ltr"
                >
                  {phone2.value}
        ...
        <Container>
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:items-start lg:gap-16">
            <div className="min-w-0">
              ...
              <h2 className="font-bebas-neue mb-4 text-[28px] uppercase leading-tight text-brand-dark sm:text-3xl lg:text-4xl">
